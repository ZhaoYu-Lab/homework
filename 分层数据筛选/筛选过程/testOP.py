...
def data_calculate(dataList,versionSource,argu): 
    ...
    for i in range(len(dataList)):
        ...
        pccValue = np.sqrt(np.sum(np.square(np.array(dataList[i]) - np.array(dataList[argu]))))
...
def main():
    ...
    pccValueList = data_calculate(dataList,versionSource,7) # 0表示目标项目为ant1.3   1表示ant1.4   2表示ant1.5    3表示ant1.6  4表示ant.7
    tarketCamel_1_4 = []
    tarketCamel_1_4 = result_version(pccValueList,tarketCamel_1_4,'camel',9) #29表示选取30个相似的项目 4表示取5个相似的源项目 
    #print(tarketAnt_1_4)
    resultSource = mergeV(tarketCamel_1_4,len(tarketCamel_1_4)) #接收合并的源项目集合
    #print(resultSource)#输出resultSource里面是距离目标tarketLog4j_1_0项目最近的五个最相似的项目
    tarketVersion=readCsv(camel_1_4_data_source) #读取目标项目文件,返回处理过 Y N 标签的目标项目文件
    ...
    resultTrain.to_csv('G:/Zhaoyu-Experiment/Test-Second/o--pcc/10--5/trainCamel-1.4.csv',index=False)
    testData = pd.DataFrame(columns=attr,data=tarketVersion)#导出数据 数据有三列，列名分别为attr
    testData.to_csv('G:/Zhaoyu-Experiment/Test-Second/o--pcc/10--5/testCamel-1.4.csv',index=False)
