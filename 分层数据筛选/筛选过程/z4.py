...
中心趋势度量和分布形状度量（均值，中位数，偏度，峰度）作为衡量项目相似性的度量分布，最后选用余弦距离公式选择k个距离目标项目最近的源项目

df[''] = np.log(df[''].values+1)#对所有数据进行log转换，其中将0值+1

dist3=np.dot(vector1,vector2)/(np.linalg.norm(vector1)*(np.linalg.norm(vector2)))#余弦距离
...
def cal_dist(versionS,versionT):
    vector1 = np.array(versionS)
    vector2 = np.array(versionT)
...
#目标项目的余弦距离排序结果 回避0 1 2 3 4   23-pbeans  24-pbeans2
...
k=10时，vOrder[15],vOrder[14],vOrder[16],vOrder[13],vOrder[29],vOrder[12],vOrder[7],vOrder[18],vOrder[19],vOrder[39]
...
# #目标项目的余弦距离排序结果 回避5 6 7 8   23-pbeans  24-pbeans2
...
#  #目标项目的余弦距离排序结果 回避9 10 11 23-pbeans  24-pbeans2
...
#  #目标项目的余弦距离排序结果 回避12 13 14 15 16 23-pbeans  24-pbeans2
...
#  #目标项目的余弦距离排序结果 回避17 18 19  23-pbeans  24-pbeans2
...
#  #目标项目的余弦距离排序结果 回避20 21 22 23-pbeans  24-pbeans2
...
#  #目标项目的余弦距离排序结果 回避25 26 27 28 23-pbeans  24-pbeans2
...
#  #目标项目的余弦距离排序结果 回避29 30 31 23-pbeans  24-pbeans2
...
#  #目标项目的余弦距离排序结果 回避32  23-pbeans  24-pbeans2
...
#  #目标项目的余弦距离排序结果 回避33 34 35  23-pbeans  24-pbeans2
...
#  #目标项目的余弦距离排序结果 回避36 37 38 39  23-pbeans  24-pbeans2
...
#  #目标项目的余弦距离排序结果 回避40 41 42 23-pbeans  24-pbeans2
